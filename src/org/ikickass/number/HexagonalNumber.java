...
public class HexagonalNumber extends InternalPolygonalNumber
{
    private static HexagonalNumber         MY_INSTANCE;
    private final Map<Integer, BigInteger> MY_NUMBERS;

    private HexagonalNumber()
    {
    ...
    protected void calculate(int n)
    {
        if(!MY_NUMBERS.containsKey(n))
        {
            ...
            final BigInteger current = N.multiply(N.multiply(BigInteger.TWO).subtract(BigInteger.ONE));
            synchronized(MY_NUMBERS)
            {
    ...
    public static HexagonalNumber getInstance()
    {
        if(MY_INSTANCE == null) MY_INSTANCE = new HexagonalNumber();
        return MY_INSTANCE;
    }

    public static BigInteger getHexagonalNumber(final int i)
    {
    ...
    {
        HexagonalNumber.stream(100).forEach(n -> System.out.println(n));
    }
}
